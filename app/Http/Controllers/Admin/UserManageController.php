    ...
    public function saveBalanceByUsers(Request $request)
    {
        ...
        try {
            ...
            if ($request->operation == "on") {
                $trx->outcome = $request->amount;
                $user->balance += $request->amount;
                $user->save();
                $txt = $request->amount . ' ' . ' credited in your account.' . '<br>' . $request->message;
//            notify($user, 'Credited Your Account', $txt);
                $message = $request->amount . ' credited to ' . $user->name . ' By ' . Auth::guard('admin')->user()->username.' : '.$request->message;
            } else {
                if ($user->balance > $request->amount) {
                    $trx->income = $request->amount;
                    $user->balance -= $request->amount;
                    $user->save();
                    $txt = $request->amount . ' ' . ' debited in your account.' . '<br>' . $request->message;
//                notify($user, 'Debited Your Account', $txt);
                    $message = $request->amount . ' ' . ' debited From ' . $user->name . ' By ' . Auth::guard('admin')->user()->username.' : '.$request->message;;
                } else {
...
//dd($request);
        ...
        if($request->balance && $request->balance>0)
        {
            ...
            $trx->balance = $user->balance;
            $trx->details = $user->balance . ' ' . ' debited From ' . $user->name . ' By ' . Auth::guard('admin')->user()->username;
            $trx->save();
        }
